...
public:
	...
	bool Update(float dt) {
		...
		// Efekt pulsowania tylko dla heksagonów
		if (sides == 6) {
			pulseTime += dt;
			pulseScale = 1.0f + 0.1f * sinf(pulseTime * 3.0f); // 10% pulsacja z częstotliwością 3 Hz

			color.r = (unsigned char)(127 + 127 * sinf(pulseTime * 2.0f));
			color.g = (unsigned char)(127 + 127 * sinf(pulseTime * 2.0f + 2.0f));
			color.b = (unsigned char)(127 + 127 * sinf(pulseTime * 2.0f + 4.0f));
		}
		else {
			pulseScale = 1.0f;
		}
...
public:
	PentagonAsteroid(int w, int h) : Asteroid(w, h) {
		baseDamage = 15; sides = 5; physics.velocity = Vector2Scale(physics.velocity, 1.25f);
	}
	void Draw() const override {
...
public:
	HexagonAsteroid(int w, int h) : Asteroid(w, h) {
		baseDamage = 20; sides = 6; physics.velocity = Vector2Scale(physics.velocity, 1.5f);
		color = Color{
	...
	void Draw() const override {
		Renderer::Instance().DrawPoly(transform.position, 6, GetRadius() * pulseScale, transform.rotation, color);
	}
};
...
public:
	...
	void Draw() const {
		if (type == WeaponType::BULLET) {
			...
			DrawCircleLines(transform.position.x, transform.position.y, 10.f, RED);
		}
	}
	...
	float GetRadius() const {
		...
		if (type == WeaponType::MISSILE) return 8.f;
		return 2.f;
	}
...
public:
	...
	float GetFireRate(WeaponType wt) const {
		...
		case WeaponType::MISSILE: return fireRateMissile;
		default: return 1.0f;
		}
	}
	...
	float GetSpacing(WeaponType wt) const {
		...
		case WeaponType::MISSILE: return spacingMissile;
		default: return 20.f;
		}
	}
	...
	void Run() {
		...
		while (!WindowShouldClose()) {
			...
			// Render everything
			{
				...
				case WeaponType::MISSILE: weaponName = "MISSILE"; break;
